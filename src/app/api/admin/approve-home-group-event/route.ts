  ...
  try {
    ...
    if (!event_id || !action) {
      return NextResponse.json(
        { success: false, message: 'Missing event_id or action' },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json({ success: true, message: `Home group event ${action}` })
  } catch (error) {
    console.error('Approve home group event error:', error)
